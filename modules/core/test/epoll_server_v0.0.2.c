...
void* server_make() {
	...
	int serfd = create_socket(8000);
	int serfd1 = create_socket(9000);
	fds_t fds[2];
	fds[0].fd = serfd;
	fds[1].fd = serfd1;
	int fds_num = 2;
	...
	for(int k=0; k<fds_num; k++) {
		...
		ev.data.u32 = k;
		epoll_ctl(epoll_fd, EPOLL_CTL_ADD, fds[k].fd, &ev);
	}
	...
	for(;;) {
		...
		for(int i=0; i<evnum; i++) {
			...
			} else if( tempfd <= fds_num ) {
				handle_accept(fds[tempfd].fd, epoll_fd);
